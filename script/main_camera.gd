...
var _is_panning: bool = false  # New variable for panning state
...
func _input(event):
	...
	# Handle touchpad gesture events (two-finger pan for zoom)
	elif event is InputEventPanGesture:
		_target_zoom = clamp(_target_zoom + event.delta.y * zoom_speed, min_zoom, max_zoom)
